...
public class HistorySingleActivity extends AppCompatActivity implements OnMapReadyCallback, DirectionCallback {
                ...
                if(dataSnapshot.exists()){
                    for (DataSnapshot child:dataSnapshot.getChildren()){
                        ...
                        if (child.getKey().equals("price")){
                            ridePrice = Double.valueOf(child.getValue().toString());
                            mPay.setText(getResources().getString(R.string.pay_here) + " - " + child.getValue().toString().substring(0, 5));
                        }
    ...
    private void payPalPayment() {
        PayPalPayment payment = new PayPalPayment(new BigDecimal(ridePrice), "USD", "Lamper Ride",
